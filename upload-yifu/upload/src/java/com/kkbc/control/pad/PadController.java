...
public class PadController {
	...
	public void saveScore(HttpServletResponse response,HttpServletRequest request) throws IOException {
		...
		if (StringUtils.isNotEmpty(score)) {
			logger.info("上传分数信息:"+score);
			String info = URLDecoder.decode(score, "utf-8");
			
			JSONObject scoreObject=JSONObject.parseObject(info);
			
			YifuTest scoreInfo= (YifuTest) JSONObject.toJavaObject(scoreObject, YifuTest.class);
			jsonObject.put("state", testService.saveStuScore(scoreInfo));
		}else {
			logger.info("没有上传的分数信息;");
			jsonObject.put("state", 3);
		}
	...
	public void getInfo(HttpServletResponse response,HttpServletRequest request) throws IOException {
		...
		jsonObject.put("info", Consts.receiveMsg);
	...
	public static void main(String[] args) {
		YifuTest test= new YifuTest();
		test.setComment1("备注1");
		test.setComment2("备注2");
		test.setContent("内容");
		test.setLqid("1-1");
		test.setPad_name("pad名");
		test.setQid("1");
		test.setRight_answer("正确答案");
		test.setScore(5f);
		test.setSid("1000001");
		test.setStu_answer("学生答案");
		test.setType(1);
		test.setUuid("uuid");
		test.setWd1("维度1");
		test.setWd2("维度2");
