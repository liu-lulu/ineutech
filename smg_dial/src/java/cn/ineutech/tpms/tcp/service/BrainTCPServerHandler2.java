 ...
 * @author liululu
 */
public class BrainTCPServerHandler2 extends ChannelInboundHandlerAdapter {
...
//		this.deviceLoginLogService=(DeviceLoginLogServiceImpl) SpringContextUtils.context.getBean("deviceLoginLogService");
	...
	public void channelRead(ChannelHandlerContext ctx, Object msg) throws InterruptedException {
		try {
			...
			logger.info("本次接收数据长度:"+bs.length+";数据:"+HexStringUtils.toHexString(bs)+";总共未处理数据"+HexStringUtils.toHexString(session.getRowData()));
...
//			}
				...
				byte[] shengyu = null;
				byte[] proData = packageUtil.getProData(rowData, shengyu);
				if (proData!=null&&proData.length==36) {
					QueueElement element = new QueueElement(ctx.channel(), proData);
					element.setDeviceId(session.getDeviceId());
					element.setType(QueueElement.TYPE_DATA);
					this.msgQueueService.push(element);
					session.setRowData(shengyu);
				}
			}
