					...
					sendToDevice.sendToMonitor(sendToDevice.sendDataToMonitor(
						...
						if (bindNo.length == 2) {
							...
							if (hard == null || StringUtils.isEmpty(hard.getMac())) {
								Hard brainHard = service.getBrainByNo(Integer.valueOf(brainNo));
								if (brainHard == null) {
									// 该编号没有对应的脑电 
									result=TPMSConsts.BRAIN_BIND_FAIL;
									comment=TPMSConsts.BRAIN_BIND_FAIL_2;
								} else {
									int ret = service.userBindBrain(Integer.valueOf(humanId), brainHard);
									if (ret==1) {
										comment=comment+brainHard.getHardId();
									} else if (ret==2) {
										// 该脑电已绑定到其他人员,不能重复绑定
										result=TPMSConsts.BRAIN_BIND_FAIL;
